...
class CRLRewardManager:
    ...
    def __init__(self, tokenizer, num_examine, compute_score=None, run_id=None, reward_fn_key='data_source') -> None:
        ...
        self.add_one_output_penalty = True
    ...
    def get_cft_data_score(self, data: DataProto, response_str, prompt_str, extracted_answers, acecoder_correctness):
        ...
        for extracted_answer in response_str:
            ...
            if matches:
                if 'T' in matches[-1]:
                    ...
                    print('undefined symbol', matches[-1], matches)
                    results.append(None)
            else:
                results.append(None)
        ...
        for result, correctness, score in zip(results, acecoder_correctness, scores):
            if result is not None:
                if result == correctness:
                    score['pass_rate'] = 1.0
                    score['binary_pass_rate'] = 1.0
                    score['score'] = 1.0
                else:
                    score['pass_rate'] = 0.0
                    score['binary_pass_rate'] = 0.0
                    score['score'] = 0.0
                # if result == penalty_symbol:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        for i in range(len(data)):
            if data[i].non_tensor_batch['extra_info'].get("correctness") is not None:
                test_cases.append(data[i].non_tensor_batch['extra_info']["correctness"])
                cft_data_idxs.append(i)
            elif data[i].non_tensor_batch['extra_info'].get("inputs_outputs"):
        ...
        print('len(prime_code_data_idxs): ', len(prime_code_data_idxs))
        print('len(cft_data_idxs): ', len(cft_data_idxs))
        print('len(acecoder_data_idxs): ', len(acecoder_data_idxs))
        
        # for i in range(len(data)):
        #     correctness.append(data[i].non_tensor_batch['extra_info']['correctness'])
        ...
        if len(cft_data_idxs) > 0:
            cft_data = data[cft_data_idxs]
            cft_response_str = [response_str[i] for i in cft_data_idxs]
            cft_prompt_str = [prompt_str[i] for i in cft_data_idxs]
            cft_correctness = [test_cases[i] for i in cft_data_idxs]
            cft_extracted_answers = [extracted_answers[i] for i in cft_data_idxs]
            cft_scores = self.get_cft_data_score(cft_data, cft_response_str, cft_prompt_str, cft_extracted_answers, cft_correctness)
            print('cft_scores: ', cft_scores[:20])
            print(f"Step {self.step_idx}: {len(cft_data_idxs)} cft data scores")
            ...
            print(" - Average cft score: ", sum([x['score'] for x in cft_scores]) / len(cft_scores))
        else:
            cft_scores = []
        ...
        idxs_map = sorted([(idx, i, 'prime') for i, idx in enumerate(prime_code_data_idxs)] + [(idx, i, 'cft') for i, idx in enumerate(cft_data_idxs)], key=lambda x: x[0])
        for i in range(len(data)):
            if idxs_map[i][2] == "prime":
                scores[i] = prime_code_scores[idxs_map[i][1]]
            else:
                scores[i] = cft_scores[idxs_map[i][1]]
